...
export default {
  ...
  props: {
    ...
    fields: {
      type: Array,
      default () {
        return ['#', 'supplier_name', 'contact_number', 'address', 'status', 'action']
      }
    },
    caption: {
      type: String,
  ...
  methods: {
    ...
    getValueDel(data){
      if (confirm('Are you sure you want to delete ' + data.supplier_name +'?')) {
          this.$store.dispatch('supplier/deleteSupplier', data.id);
      }
