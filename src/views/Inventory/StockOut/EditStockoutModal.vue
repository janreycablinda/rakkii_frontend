...
export default {
    ...
    props: ['editStockinModalData'],
    watch: {
        editStockinModalData(newData){
            
            this.editModalShow = true;
            this.form.customer_id = {
                label: newData.data.customer.company_name,
                value: newData.data.customer.id
            }
            this.form.id = newData.data.id;
            this.form.purchase_date = new Date(newData.data.purchase_date);
            this.form.delivery_date = new Date(newData.data.delivery_date);

            let params = [];
            newData.data.stockout_temps.forEach(item => {
                if(item.item.description){
                    params.push({
                        item_id: item.item_id,
                        item: item.item.item + ' ' + item.item.description + '-' + item.item.brand,
                        qty: item.qty,
                        unit: item.item.unit
                    });
                }else{
                    params.push({
                        item_id: item.item_id,
                        item: item.item.item + ' ' + item.item.brand,
                        qty: item.qty,
                        unit: item.item.unit
                    });
                }
            ...
            this.$store.commit('stockout/SET_STOCKOUT_TEMP', params);
        },
        purchase_date(){
            ...
            if (this.isValid) {
                if(this.stockin_temp != ''){
                    ...
                    const params = {
                     ...
                     this.$store.dispatch('stockout/updateStockout', params).then(() => {
